...
def summarize_multithread(pool, variable):
    ...
    files = glob(os.path.join(home, "Data", "Climate", "gridmet", "tifs", "*" + variable  + "_AMJJA" + "*.tif"))
    ...
    p.close()
    p.join()
    df_all.to_csv(os.path.join(home, "Data", "Climate", "gridmet","Summary", 'gs_' + variable + ".csv"))
